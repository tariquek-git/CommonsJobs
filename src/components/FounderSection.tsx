    ...
    <div className="space-y-4">
      ...
      <div ref={stepsRef} className={`rounded-2xl border overflow-hidden ${cardBg}`}>
        <div className="px-5 sm:px-8 py-6 sm:py-8">
          <div className="flex items-start justify-between sm:justify-center sm:gap-0">
            ...
            >
              ...
              <div
                className={`w-6 sm:w-12 h-0.5 ${dark ? 'bg-gradient-to-r from-white/30 to-white/20' : 'bg-gradient-to-r from-brand-400 to-[#FF3B8B]'}`}
              />
              <div
                className={`h-1.5 w-1.5 rounded-full ${dark ? 'bg-white/40' : 'bg-[#FF3B8B]'}`}
              />
            </div>
            ...
            >
              ...
              <p className={`text-xs ${textMuted} mt-0.5 hidden sm:block`}>
                AI cuts through corporate speak
              </p>
            </div>
            ...
            >
              ...
              <div
                className={`w-6 sm:w-12 h-0.5 ${dark ? 'bg-gradient-to-r from-white/30 to-white/20' : 'bg-gradient-to-r from-[#FF3B8B] to-emerald-400'}`}
              />
              <div
                className={`h-1.5 w-1.5 rounded-full ${dark ? 'bg-white/40' : 'bg-emerald-400'}`}
              />
            </div>
            ...
            >
              ...
              <div
                className={`w-6 sm:w-12 h-0.5 ${dark ? 'bg-gradient-to-r from-white/30 to-white/20' : 'bg-gradient-to-r from-emerald-400 to-[#7B61FF]'}`}
              />
              <div
                className={`h-1.5 w-1.5 rounded-full ${dark ? 'bg-white/40' : 'bg-[#7B61FF]'}`}
              />
            </div>
        ...
        <div className={`border-t ${borderColor}`}>
          ...
          <div
            className={`overflow-hidden transition-all duration-300 ease-out ${detailsOpen ? 'max-h-[600px] opacity-100' : 'max-h-0 opacity-0'}`}
          >
            <div className="px-5 sm:px-8 pb-5 space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div
                  className={`rounded-xl p-3 ${dark ? 'bg-white/[0.06] border border-white/10' : 'bg-brand-50/50 border border-brand-100'}`}
                >
                  <p
                    className={`text-xs font-semibold mb-1 ${dark ? 'text-brand-300' : 'text-brand-700'}`}
                  >
                    For posters
                  </p>
                  <p className={`text-xs ${textBody} leading-relaxed`}>
                    Paste a job URL or description. AI rewrites the corporate speak into real talk —
                    what someone in the role would actually care about. You review and edit before
                    submitting.
                  </p>
                </div>
                <div
                  className={`rounded-xl p-3 ${dark ? 'bg-white/[0.06] border border-white/10' : 'bg-emerald-50/50 border border-emerald-100'}`}
                >
                  <p
                    className={`text-xs font-semibold mb-1 ${dark ? 'text-emerald-300' : 'text-emerald-700'}`}
                  >
                    For candidates
                  </p>
                  <p className={`text-xs ${textBody} leading-relaxed`}>
                    Browse real roles, humanized. Apply directly or request a warm intro — I'll pass
                    your info to the poster. They decide if they want to connect. No pressure on
                    either side.
                  </p>
                </div>
              </div>
              <div
                className={`rounded-xl p-3 ${dark ? 'bg-white/[0.06] border border-white/10' : 'bg-gray-50 border border-gray-200/60'}`}
              >
                <p
                  className={`text-xs font-semibold mb-1 ${dark ? 'text-white/80' : 'text-gray-700'}`}
                >
                  The warm intro promise
                </p>
                <p className={`text-xs ${textBody} leading-relaxed`}>
                  Candidates won't know if you respond or not. All I ask is that if you see
                  potential, give them a real look. And if you can help regardless — even just a
                  pointer or referral — please do. That's what makes this different from every other
                  job board.
                </p>
              </div>
            </div>
